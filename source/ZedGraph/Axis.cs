...
namespace ZedGraph
{
	...
	/// <version> $Revision: 3.61 $ $Date: 2006-03-27 01:31:37 $ </version>
	[Serializable]
	...
	#region Defaults
		...
		public struct Default
		{
			...
			/// <summary>
			/// The default fill mode for filling in the <see cref="Title" /> _text background
	...
	#region Constructors
		...
		/// </summary>
		/// <param name="title">A string containing the axis _title</param>
	...
	#region Scale Properties
		...
		{
			get { return _cross; }
			set { _cross = value; this._crossAuto = false; }
		}
		/// <summary>
	...
	#region Type Properties
		...
		public AxisType Type
		{
			...
			set { this._scale = Scale.MakeNewScale( this._scale, value ); }
		}
	...
	#region Rendering Methods
		...
		/// <see cref="_tmpSpace" /> for use by the <see cref="GraphPane.CalcAxisRect(Graphics)" />
		/// method.
		/// </remarks>
		/// <param name="g">
		/// A graphic device object to be drawn into.  This is normally e.Graphics from the
		...
		public float CalcSpace( Graphics g, GraphPane pane, float scaleFactor, out float fixedSpace )
		{
			...
			if ( this._isVisible )
			{
				...
				// space between scale label and axis _label is 0.5 tic
			...
			_tmpSpace = Math.Max( _tmpSpace, this._minSpace * (float)scaleFactor );
				...
				float left, float right )
		{
					...
					this._scale._min < 0.0 && this._scale._max > 0.0 )
			{
				...
				Pen zeroPen = new Pen( this._color,
								...
								float scaleFactor, float topPix )
		{
					...
					&& this._isVisible )
			{
				...
				if ( this._scale.IsLog || tMinor < tMajor )
				{
					...
					double dVal = first;
					float pixVal;
					Pen pen = new Pen( this._color, pane.ScaledPenWidth( MinorTic._penWidth, scaleFactor ) );
		...
		public void DrawTitle( Graphics g, GraphPane pane, float shiftPos, float scaleFactor )
		{
			...
			if ( this._isVisible && this._title._isVisible && str.Length > 0 )
			{
