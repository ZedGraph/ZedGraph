...
namespace ZedGraph
{
	...
	/// <version> $Revision: 3.60 $ $Date: 2006-03-27 01:31:37 $ </version>
	[Serializable]
	...
	#region Private Fields
		...
		/// data _points based on any manually set scale range values.  Use the public property
		...
		/// for the individual types _available.
	...
	#region Defaults
		...
		public new struct Default
		{
			...
			/// true to have the auto-scale-range code ignore the initial data _points
	...
	#region General Properties
		...
		/// </summary>
		/// <remarks>If true, then initial data _points where the Y value
		...
		/// subset the data _points based on any manually set scale range values.</summary>
		/// <remarks>The bounds provide a means to subset the data.  For example, if all the axes are set to
		/// autoscale, then the full range of data are used.  But, if the XAxis.Min and XAxis.Max values
		/// are manually set, then the Y data range will reflect the Y values within the bounds of
		/// XAxis.Min and XAxis.Max.  Set to true to subset the data, or false to always include
		/// all data _points when calculating scale ranges.</remarks>
	...
	#region Constructors
			...
			: base( title, rect )
		{
			...
			this._barSettings = new BarSettings();
			...
			: base( rhs )
		{
			...
			this._barSettings = rhs._barSettings.Clone();
	...
	#region Serialization
		...
		/// </summary>
		//changed to 2 when _yAxisList and _y2AxisList were added
		//changed to 3 when _chart object was added
	...
	#region Rendering Methods
		...
		public void AxisChange( Graphics g )
		{
			...
			{
				//don't want to display axis or _border if there's only pies
		...
		{
			// Calculate the axis _rect, deducting the area for the scales, titles, _legend, etc.
			//int		hStack;
			//float	legendWidth, legendHeight;

			// Draw the pane _border & background _fill, the _title, and the GraphObj objects that lie at
			// ZOrder.G_BehindAll
			base.Draw( g );
			...
			if ( this._chart._isRectAuto )
			{
				this._chart._rect = CalcAxisRect( g, scaleFactor );
				//this.pieRect = PieItem.CalcPieRect( g, this, scaleFactor, this.axisRect );
			}
			else
			...
			if ( showGraf )
				this._graphObjList.Draw( g, this, scaleFactor, ZOrder.F_BehindChartFill );
			...
			{
				// Draw the GraphItems that are behind the Axis objects
				this._graphObjList.Draw( g, this, scaleFactor, ZOrder.E_BehindAxis );
				...
				this._graphObjList.Draw( g, this, scaleFactor, ZOrder.D_BehindCurves );

				// Clip the _points to the actual plot area
				g.SetClip( this._chart._rect );
				this._curveList.Draw( g, this, scaleFactor );
				g.SetClip( this._rect );

				// Draw the GraphItems that are behind the Axis _border
				this._graphObjList.Draw( g, this, scaleFactor, ZOrder.C_BehindChartBorder );
			}

			// Border the axis itself
			this._chart.Border.Draw( g, this.IsPenWidthScaled, scaleFactor, this._chart._rect );
		...
		/// is the plot area bounded by the axes, and the _rect is the total area as
		...
		/// PaintEventArgs argument to the Paint() method.
		/// </param>
		/// <returns>The calculated axis _rect, in pixel coordinates.</returns>
		...
		{
			// Axis _rect starts out at the full pane _rect less the margins
			//   and less space for the Pane _title
			...
			// actual minimum axis space for the left side of the Axis _rect
			float minSpaceL = 0;
			// actual minimum axis space for the right side of the Axis _rect
			float minSpaceR = 0;
			// actual minimum axis space for the bottom side of the Axis _rect
			float minSpaceB = 0;
			// actual minimum axis space for the top side of the Axis _rect
			float minSpaceT = 0;
			...
			SetSpace( this._xAxis, clientRect.Height - this._xAxis._tmpSpace, out spaceB, out spaceT );
		...
		/// required to fit the tic marks, scale labels, and axis _title.
	...
	#region AddCurve Methods
		...
		/// <param name="points">A <see cref="IPointList"/> of double precision value pairs that define
		/// the X and Y values for this curve</param>
		/// <param name="color">The color to used to _fill the bars</param>
		...
		/// the <see cref="ZedGraph.CurveList.Add"/> method.
		/// </summary>
		/// <param name="label">The _text label (string) for the curve that will be
		/// used as a <see cref="Legend"/> entry.</param>
		...
		/// <summary>
		/// Add a <see cref="PieItem"/> to the display, providing a gradient _fill for the pie color.
		/// </summary>
	...
	#region General Utility Methods
		...
		{
			// Setup the scaling data based on the axis _rect
			this._xAxis.Scale.SetupScaleData( this, this._xAxis );
			foreach ( Axis axis in _yAxisList )
				axis.Scale.SetupScaleData( this, axis );
			foreach ( Axis axis in _y2AxisList )
		...
		/// </remarks>
		/// <param name="title">The _title for the <see cref="YAxis" />.</param>
		...
		public int AddYAxis( string title )
		{
			...
			return this._yAxisList.Add( axis );
		}
		...
		public int AddY2Axis( string title )
		{
			...
			axis.MajorTic.IsInside = false;
			axis.MinorTic.IsInside = false;
			return this._y2AxisList.Add( axis );
		}
		...
		/// Note: If the pane _title is selected, then the <see cref="GraphPane"/> object
		/// will be returned.
		/// </param>
		...
		/// index will be -1 if no data _points are available.</param>
			...
			out object nearestObj, out int index )
		{
			...
			if ( AxisRangesValid() )
			{
				...
				// See if the point is in the _legend
				...
				if ( saveZOrder <= ZOrder.G_BehindAll && this._title._isVisible )
				{
					SizeF size = this._title._fontSpec.BoundingBox( g, this._title._text, scaleFactor );
					tmpRect = new RectangleF( ( this._rect.Left + this._rect.Right - size.Width ) / 2,
				...
				tmpRect = new RectangleF( tmpAxisRect.Left, tmpAxisRect.Bottom,
					tmpAxisRect.Width, this._rect.Bottom - tmpAxisRect.Bottom );
				if ( saveZOrder <= ZOrder.E_BehindAxis && tmpRect.Contains( mousePt ) )
				{
		...
		/// </summary>
		/// <remarks>
		/// This method will search only through the _points for the specified
		/// curve to determine which point is
		/// nearest the mouse point.  It will only consider _points that are within
		/// <see cref="Default.NearestTol"/> pixels of the screen point.
		/// </remarks>
		/// <param name="mousePt">The screen point, in pixel coordinates.</param>
		/// <param name="targetCurve">A <see cref="CurveItem"/> object containing
		/// the data _points to be searched.</param>
		...
		/// no data _points are available.</param>
		...
		/// .  iNearest will
		/// be -1 if no data _points are available.</param>
		...
		/// nearest.  It will only consider _points that are within
		/// <see cref="Default.NearestTol"/> pixels of the screen point.
		/// </remarks>
		/// <param name="mousePt">The screen point, in pixel coordinates.</param>
		/// <param name="nearestCurve">A reference to the <see cref="CurveItem"/>
		...
		public bool FindNearestPoint( PointF mousePt,
			out CurveItem nearestCurve, out int iNearest )
		{
			return FindNearestPoint( mousePt, this._curveList,
				out nearestCurve, out iNearest );
		}
		...
		{
			return BarBaseAxis()._scale.GetClusterWidth( this );
		}
		...
		{
			Axis barAxis;
			if ( this._barSettings.Base == BarBase.Y )
				barAxis = this.YAxis;
			else if ( this._barSettings.Base == BarBase.Y2 )
				barAxis = this.Y2Axis;
			else
				barAxis = this.XAxis;
