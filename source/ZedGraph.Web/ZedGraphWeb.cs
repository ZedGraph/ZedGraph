...
namespace ZedGraph.Web
{
	...
	#region Attributes
		...
		[Category("Axis")]
		[NotifyParentProperty(true)]
		public bool IsChartRectAuto
		{
			get 
			{
				object x = ViewState["IsChartRectAuto"]; 
				return (null == x) ? true : (bool)x;
			}
			set { ViewState["IsChartRectAuto"] = value; }			
		}
		...
		public ZedGraphWebRect ChartRect
		{
	...
	#region Map Embedded Content
		...
		protected void SetWebProperties( Graphics g, GraphPane pane )
		{
			try 
			{
                ...
                pane.BarSettings.ClusterScaleWidth = this.ClusterScaleWidth;								
				...
				this.ChartRect.CopyTo( pane.Chart.Rect );
				pane.Chart.IsRectAuto = this.IsChartRectAuto;
